      ...
      <div class="content-container" style="padding-top: 0">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <router-link
                :to="`/source/index?groupName=${scope.row.name}`"
              >
                <el-button size="mini">查看素材</el-button>
              </router-link>
            </template>
          </el-table-column>
        </el-table>
...
<script>
import { getSourceGroupList, updateSourceGroup, deleteSourceGroup } from '@/api/source'
  ...
  methods: {
    deleteGroupDevice() {
      ...
      this.$confirm('确定要删除吗？', '提示', {
        ...
        ids = ids.join(',')
        deleteSourceGroup({ ids }).then(res => {
    ...
    getMainTableData() {
      ...
      getSourceGroupList(_form).then(response => {
